...
export default class GraphClaim extends React.Component {
    ...
    renderChildren(childClaims) {
        ...
        childClaims.forEach(child => {
            childrenMarkup.push(<GraphClaim key={child.claim._key} claimStore={child} />);
        });
        return <g>{childrenMarkup}</g>;
    }

    render() {
        return (
            <Group className="graph-claim" x={this.props.claimStore.x} y={this.props.claimStore.y}>
                <Group
